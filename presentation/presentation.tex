...
\begin{frame}[fragile]{Random Variables and Probability Distributions}
	...
	\begin{easylist}[itemize]
		...
		## Discrete
		## Continuous
		# A probability distribution is a mathematical function that gives the probabilities of occurrence of of different possible outcomes
	\end{easylist}
...
\begin{columns}
	\begin{column}{0.5\textwidth}
		\begin{easylist}[itemize]
			...
			# Percentiles:
			##  $Pxx$ in a distribution is the value such that the probability is $xx$ \% to obtain values $Pxx$ or less
...
\begin{frame}[fragile]{Expected Value, P50 or Most Likely Value}
	\begin{columns}
		\begin{column}{0.5\textwidth}
			\begin{easylist}[itemize]
			# Expected value (mean): Center of gravety of the density
			\begin{equation*}
				\mu = E(X) = \int xg(x)\,dx
			\end{equation*}
...
\begin{frame}[fragile]{Computational Rules}
	...
	\begin{frame}[fragile]{Statistical Concepts}
		\begin{easylist}[itemize]
			#	\textbf{Probabilities, Random Variables and Distributions} 
			#	\textbf{PDF and CDF}
